...
const PFPDownloader = () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          throw new Error(errorData.error || 'Invalid YouTube channel URL format.');
      ...
      if (!data.url) {
        throw new Error('No profile picture found for this channel.');
      }
      
      setPfpUrl(data.url);
      setError('');
    } catch (err: unknown) {
      let errorMessage = 'Failed to extract profile picture.';
      
      if (err instanceof Error) {
        if (err.name === 'AbortError') {
          errorMessage = 'Request timed out. Please try again.';
        } else if (err.message.includes('Failed to fetch')) {
          errorMessage = 'Network error. Please check your internet connection or try again.';
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Create filename from channel name or use default
      const filename = input.includes('@') 
        ? `${input.split('/@').pop()?.split('/')[0]?.split('?')[0] || 'channel'}-profile-picture.jpg`
        : input.includes('/channel/')
        ? `${input.split('/channel/').pop()?.split('/')[0]?.split('?')[0] || 'channel'}-profile-picture.jpg`
        : input.includes('/c/')
        ? `${input.split('/c/').pop()?.split('/')[0]?.split('?')[0] || 'channel'}-profile-picture.jpg`
        : `${input.replace(/[^a-zA-Z0-9]/g, '')}-profile-picture.jpg`;
      ...
      setError('Failed to download profile picture. Please try again.');
    } finally {

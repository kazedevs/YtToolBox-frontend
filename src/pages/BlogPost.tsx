...
const BlogPost = () => {
  ...
  // Use a more robust approach for MDX loading
    ...
    if (post.mdxPath) {
      const loadMDX = async () => {
        try {
          ...
          // Use Vite's glob import for reliable dynamic imports
          const modules = import.meta.glob('/src/content/*.mdx');
          const fileName = post.mdxPath.split('/').pop();
          const modulePath = `/src/content/${fileName}`;
          
          console.log('Looking for MDX file:', modulePath);
          console.log('Available modules:', Object.keys(modules));
          
          if (modules[modulePath]) {
            const module = await modules[modulePath]();
            setMDXContent(() => (module as { default: React.ComponentType }).default);
            console.log('Successfully loaded MDX:', modulePath);
          } else {
            throw new Error(`MDX file not found: ${modulePath}. Available: ${Object.keys(modules).join(', ')}`);
          }
        } catch (error) {
          console.error('Failed to load MDX:', error);
          setMdxError(`Failed to load blog content: ${error instanceof Error ? error.message : 'Unknown error'}`);
        } finally {
          setMdxLoading(false);
        }
      };
            ...
            >
              {mdxError ? (
                ...
                <Typography variant="body1" sx={{ whiteSpace: "pre-line" }}>
                  {post.content || "Content loading failed. Please refresh the page."}
                </Typography>
              )}
            </Box>
          </Box>
